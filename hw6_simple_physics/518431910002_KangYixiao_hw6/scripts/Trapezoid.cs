...
class Trapezoid : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
		...
		float thetaTmp = theta + omega * t*step;
        float omegaTmp = omega -g*Mathf.Sin(theta)/length*t*step;
        float deltaTheta = step*(omega+omegaTmp)*t/2;
        omega = omega - g*(Mathf.Sin(theta)+Mathf.Sin(thetaTmp))/length*t*step/2;
