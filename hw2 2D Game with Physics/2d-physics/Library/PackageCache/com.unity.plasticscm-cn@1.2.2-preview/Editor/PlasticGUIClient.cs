...
namespace Unity.PlasticSCM.Editor
{
        ...
        IPendingChangesView, IWorkspaceWindow, IUpdateReport
    {
        ...
        public void FullCheckIn(Action endOperation)
        {
            ...
            List<ChangeInfo> changesToCheckin = mChangesTreeView.GetAllChanges();
            if (CheckEmptyOperation(changesToCheckin, HasPendingMergeLinks()))
            {
                return;
            }

            System.Action endOperations = RefreshAsset.UnityAssetDatabase;
            if (endOperation != null)
            {
                endOperations += endOperation;
            }

            mPendingChangesOperations.Checkin(
                changesToCheckin,
                new List<ChangeInfo>(){},
                "Auto save at " + DateTime.Now.ToString("MM/dd/yyyy hh:mm tt"),
                null,
                endOperations);
        }
